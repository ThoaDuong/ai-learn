/**
 * Generate B2 Level Vocabulary Seed from full-word.json
 * 
 * This script:
 * 1. Reads full-word.json and filters B2 level words
 * 2. Uses Gemini API to translate word → meaning (Vietnamese) and example → exampleTranslation (Vietnamese) 
 * 3. Outputs the generated b2-vocabulary-seed.ts file (replacing the manual version)
 * 
 * Features:
 ...
 * - Resume support
 * 
 * Run: npx tsx scripts/generate-b2-seed.ts
...
async function main() {
    ...
    // 1. Read and filter B2 words from full-word.json
    ...
    const b2Words: VocabItem[] = rawData
        .filter((entry) => entry.value.level === "B2")
        .map((entry) => {
            const v = entry.value;
    ...
    const uniqueB2Words = b2Words.filter((item) => {
        if (seen.has(item.word)) return false;
        seen.add(item.word);
        return true;
    });

    console.log(`✅ Found ${uniqueB2Words.length} unique B2 words (from ${b2Words.length} filtered entries)`);

    // 2. Check for resume progress
    const progressPath = path.resolve(__dirname, "b2-enrich-progress.json");
    ...
    if (fs.existsSync(progressPath)) {
        ...
        console.log(`♻️ Resuming from word ${startIndex + 1}/${uniqueB2Words.length}`);
    }

    // 3. Translate in batches
    const BATCH_SIZE = 30;
    const totalBatches = Math.ceil((uniqueB2Words.length - startIndex) / BATCH_SIZE);
    let batchNum = 0;

    for (let i = startIndex; i < uniqueB2Words.length; i += BATCH_SIZE) {
        batchNum++;
        const batch = uniqueB2Words.slice(i, i + BATCH_SIZE);
        ...
        console.log(`  ✅ Progress saved: ${enrichedItems.length}/${uniqueB2Words.length}`);

        if (i + BATCH_SIZE < uniqueB2Words.length) {
    ...
    // 4. Generate b2-vocabulary-seed.ts
...
`;
    ...
    const outputPath = path.resolve(__dirname, "b2-vocabulary-seed.ts");
    fs.writeFileSync(outputPath, outputContent, "utf-8");
    console.log(`\n🎉 Generated ${outputPath}`);
    console.log(`📊 Total B2 vocabulary items: ${enrichedItems.length}`);
    ...
    console.log(`  npx tsx scripts/b2-vocabulary-seed.ts`);
}
